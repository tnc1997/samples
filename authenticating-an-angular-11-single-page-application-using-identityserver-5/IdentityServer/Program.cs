                ...
                using (var configurationDbContext = scope.ServiceProvider.GetRequiredService<ConfigurationDbContext>())
                {
                    ...
                    if (!configurationDbContext.Clients.Any())
                    {
                        configurationDbContext.Clients.Add(new Client
                        {
                            ...
                            RedirectUris = new List<string> {configuration["MyAppUrl"]},
                            PostLogoutRedirectUris = new List<string> {configuration["MyAppUrl"]},
                            AllowedCorsOrigins = new List<string> {configuration["MyAppUrl"]},
                            AllowedScopes = new List<string> {IdentityServerConstants.StandardScopes.OpenId, IdentityServerConstants.StandardScopes.Profile, IdentityServerConstants.StandardScopes.OfflineAccess, "api"}
                        }.ToEntity());
